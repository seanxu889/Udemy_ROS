...
int main(int argc, char **argv)
{
	...
	cout<<"forward ?: ";
	cin>>isForward;
	move(speed, distance, isForward); //test using "rosrun beginner_tutorials r_cleaner_node"
...
void move(double speed, double distance, bool isForward)
{
	...
	double t0 = ros::Time::now().toSec(); //t0: current time
	double current_distance = 0.0;
	ros::Rate loop_rate(100);
	do{
		velocity_publisher.publish(vel_msg); //publish the velocity
		double t1 = ros::Time::now().toSec();
		current_distance = speed * (t1-t0); //estimate the current distance
		ros::spinOnce();
		loop_rate.sleep();
		//cout<<(t1-t0)<<", "<<current_distance <<", "<<distance<<endl;
	}while(current_distance < distance);
	vel_msg.linear.x =0;
	velocity_publisher.publish(vel_msg); //force the robot stop immediately
